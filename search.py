...
w_opter = torch.optim.AdamW(model.parameters(), lr=1e-4, weight_decay=1e-5)
...
def train_init(model, loader, loss_w, opt_w, epoch):
    ...
    for i in range(epoch):
        ...
        for step, batch in enumerate(loader):
            ...
            print(loss.item())
        print("Finished training on " + str(epoch) + " epoch")
        print("loss is {}".format(l / (step + 1)))

train_init(model, train_loader, loss_function_w, w_opter, 4000)
torch.save(model.state_dict(), "./model/pre_trained_20_epoch.ptb")
print("model saved")
...
#         print("w is trained completed with loss {}".format(w_loss))
...
#         print("a is trained completed with loss {}".format(a_loss))
